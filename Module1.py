...
def sol1(): 
    if Selection.get()=='Predefined':
       ...
       tr=pd.DataFrame(a)
       ts=pd.DataFrame(b)
       name='_rd_'+str(int(perc*100))+str('_')+str(seed)
       ...
       tr,ts=kmc.cal()
       name='_kmc_'+str(int(perc*100))+str('_')+str(seed)+str('_')+str(nclus)
...
def solsave2():
    ...
    name2=name+'_'+str(m)+'_'+str(int(perc*100))+'_'+str(seed)
    
    savename1= str(a_)+str(name2) + '_strbj.csv'
    st.to_csv(savename1,index=False)
    savename2= str(a_)+str(name2) + '_tsbj.csv'
    ts.to_csv(savename2,index=False)
    savename3= str(a_)+str(name2) + '_vdbj.csv'
...
Criterion_Label4.place(x=350,y=235)
Criterion_acc3.place(x=420,y=235)
#Criterion_prec3.grid(column=4, row=10, sticky=(N))
Criterion_roc3.place(x=490,y=235)
